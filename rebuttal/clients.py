...
class StripeClient:
    ...
    def charges_for_fingerprint(self, fingerprint: str) -> list[dict[str, Any]]:
        res = self.s.Charge.search(query=f"payment_method_details.card.fingerprint:'{fingerprint}' AND -disputed:'true'", limit=10)
        now = time.time()
        out = []
    ...
    def update_dispute(self, dispute_id: str, evidence: dict[str, Any], submit: bool) -> dict[str, Any]:
        return _plain(self.s.Dispute.modify(dispute_id, evidence=evidence, submit=submit))
