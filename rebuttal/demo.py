...
CUSTOMER_PHONE = os.environ.get("DEMO_CUSTOMER_PHONE", "")
...
def seed(args) -> None:
    ...
    st = StripeClient()
    s = st.s
    cust = s.Customer.create(email=CUSTOMER_EMAIL, phone=CUSTOMER_PHONE or None, name="Jordan Lee", address={"line1": "12 Ridge Rd", "city": "Boulder", "state": "CO", "postal_code": "80302", "country": "US"})
    ...
    if not args.silent_thread:
        ...
        print("no email thread seeded: the agent will place a confirmation call")
    env = open(".env", "a")
